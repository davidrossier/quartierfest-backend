...
- [x] ~~**OPEN (E1):** Kein DB-seitiger Unique-Constraint auf `(event, partei)`~~ → **Behoben (DB-002, 2026-09-08):** `uk_einladung_event_partei UNIQUE (event_id, partei_id)` per Flyway-Migration V2 und `@UniqueConstraint` auf der `Einladung`-Entity; ein zweiter POST liefert 409 «Datensatz existiert bereits.» (TC-042). Die Frontend-Logik (`nichtEingeladeneParteien`-Filter, Überspringen bestehender Einladungen) bleibt als UX-Schicht bestehen.
