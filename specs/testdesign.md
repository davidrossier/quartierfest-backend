...
- **Source**: API-001 Stufe 1 (übergreifend, kein UC)
- **Type**: Contract test
...
- [ ] **AbstractQuartierfestIT fehlt**: Das `setUp()`-Muster ist in 17 der 18 IT-Klassen identisch kopiert (`OpenApiContractIT` ausgenommen) (`tryDelete()`/Fixture-Cleanup in 14 davon, ~400+ Zeilen Boilerplate). Empfehlung: gemeinsame abstrakte Basisklasse einführen. Details → `specs/TODO.md` (TEST-001).
...
- [ ] **UC-006 PATCH für `bestaetigungVersendet`**: ~~Kein PATCH benötigt — POST/Upsert mit `id` im Body funktioniert. TC-013 aktualisiert.~~ Revidiert durch REST-001 (2026-07-09): POST-Upsert soll unterbunden werden, dafür fehlt `PUT /api/einladungen/{id}`. TC-013 testet bis dahin bewusst den Upsert. Details → `specs/TODO.md` (REST-003).
- [ ] **UC-012 PATCH für `zustellungsDatum`**: ~~Kein PATCH benötigt — POST/Upsert mit `id` im Body funktioniert. TC-032 ergänzt.~~ Revidiert durch REST-001 (2026-07-09): dafür fehlt `PUT /api/abrechnungen/{id}`. TC-032 testet bis dahin bewusst den Upsert. Details → `specs/TODO.md` (REST-003).
