...
- [ ] **OPEN (impl_status: teilweise, → `specs/TODO.md` BIZ-001):** Berechnungslogik liegt vollständig im Frontend (`abrechnungenErstellen()` in `AbrechnungenVerwaltungComponent`). Das Backend validiert nicht, ob `totalBetrag == anteilAllgemeinkosten + totalKonsumation`. Inkonsistente Beträge können direkt via `POST /api/abrechnungen` gespeichert werden. Empfehlung: Berechnung in den `AbrechnungService` verlagern (z.B. `POST /api/events/{id}/abrechnungen/berechnen`), manuelle Übersteuerung via POST validieren.
- [x] **RESOLVED (2026-09-18): Verteilschlüssel** — Der Anteil Allgemeinkosten wird **pro effektive Person** verteilt (Schritt 2–3 des Hauptflusses, Akzeptanzkriterium «Anteil Allgemeinkosten korrekt berechnen»), nicht pro Teilnahme/Partei. So im Frontend umgesetzt; die Backend-Berechnung aus BIZ-001 übernimmt diese Regel unverändert.
- [ ] **OPEN (2026-09-18): Rundung** — Das Frontend rundet kaufmännisch auf 0.01 (`rundeAufRappen()`; Backend speichert `numeric(10,2)`, DB-002). Soll auf 0.05 (Schweizer Rappenrundung) gerundet werden, und falls ja: pro Einzelbetrag oder nur auf `totalBetrag`? Entscheid mit dem Organisator vor der Backend-Umsetzung (BIZ-001) treffen.
